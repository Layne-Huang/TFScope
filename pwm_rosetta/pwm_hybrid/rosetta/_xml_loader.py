...
def fast_relax(protocols,weights_file,psipred_exe,relax_script):
    # Build SSPrediction XML fragments conditionally — must be defined
    # before the f-string below so Python can evaluate {sspred_filters}
    # and {sspred_protocols} immediately when the string is constructed.
    if psipred_exe:
        sspred_filters = f"""<SSPrediction name="pre_sspred_overall" cmd="{psipred_exe}" use_probability="0" use_svm="0" threshold="0.85" confidence="0" />
        ...
        sspred_protocols = """<Add filter="mismatch_probability" />
...
"""
    # xml_script is an f-string so all {placeholders} are already substituted.
