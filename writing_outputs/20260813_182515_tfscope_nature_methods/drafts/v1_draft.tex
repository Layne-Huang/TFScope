...
chromatin and gene-expression models\cite{stormo2000dna,weirauch2014cisbp,castromondragon2022jaspar,kulakovskiy2018hocomoco,avsec2021enformer}.
...
protein-binding microarrays\cite{berger2006compact,badis2009diversity} and
HT-SELEX\cite{jolma2013dna} have characterized many TFs, but the experiments are
laborious and have covered only a fraction of the roughly 1,600 human
TFs\cite{vaquerizas2009census,lambert2018human}, with even thinner coverage across
non-model organisms.
...
and chemistry of the interface, exploiting the well-established dependence of
recognition on DNA shape\cite{rohs2009dnashape,zhou2013dnashape,mitra2024deeppbs}.
Their accuracy, however, is gated on the availability and quality of a structure.
For a TF without a co-crystal, and for any designed protein, a complex must first be
predicted\cite{jumper2021alphafold,baek2021rosettafold,abramson2024alphafold3}, which
adds computational cost and a second, compounding source of error. Sequence-only
models avoid this dependency but have historically been less
accurate\cite{alipanahi2015deepbind,zhou2015deepsea,kelley2016basset,eraslan2019deeplearning},
and their reported advantages are frequently inflated by TF leakage, in which test
proteins share close homologs with the training set and the model succeeds by recall
rather than by genuine generalization\cite{mitra2024deeppbs}. Distinguishing real
sequence-to-specificity inference from leakage is thus as important as raw accuracy,
and has been a recurring concern in benchmarks of specificity
prediction\cite{weirauch2013evaluation}.

Protein language models (PLMs), which apply the transformer architecture and
masked-language-model pretraining of modern natural-language
processing\cite{vaswani2017attention,devlin2019bert} to evolutionary-scale sequence
data, have reshaped what is achievable from sequence alone, recovering contacts,
structure, and function that were previously thought to require experimental
input\cite{rives2021esm1b,elnaggar2021prottrans,rao2021msa,rao2019evaluating,lin2023esm2}. We reasoned that a frozen PLM might already
...
DNA, using Boltz-2\cite{passaro2025boltz2,wohlwend2024boltz1} with ColabFold multiple-sequence
...
available without supervision. This organization mirrors decades of structural
work showing that DBD families such as homeodomains and C2H2 zinc fingers
recognize DNA through characteristic, family-defining
contacts\cite{noyes2008homeodomain,najafabadi2015c2h2}, the same regularities that
explicit protein--DNA recognition codes encode by hand or learn from structural
mappings\cite{wetzel2022rclamps}. Both signals are present before any TFScope training,
...
mutation-sensitive features, such as those exploited by dedicated variant-effect
predictors\cite{meier2021language,brandes2023genome}, are required. Closing this gap without sacrificing the
...
computation. (ii) A token-level residue mixture-of-experts\cite{shazeer2017outrageously,fedus2022switch}
with two shared experts and eight routed experts, gated by a learned router that is
conditioned on a TF-family embedding, processes each residue; the shared experts capture family-invariant
...
and folded both with Boltz-2\cite{passaro2025boltz2,wohlwend2024boltz1}. Each DNA was modeled as a
