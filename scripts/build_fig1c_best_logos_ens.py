...
def main():
    ...
    _ENSC=[os.path.join(CKPT_DIR,"ckpt_best.pt")]+[f"checkpoints/iclr_phase1/v24_ens/seed{s}/ckpt_best.pt" for s in (1,7,13,23)]
    MODELS=[]
    for _ck in _ENSC:
    ...
    with torch.no_grad():
        for b in ld:
            ...
            _ps=[]; _gs=[]
            for _m in MODELS:
                gl, pl, _ = _m(b['sequence_tokens'], b['dbd_mask'], b['family_id'],
                ...
                _ps.append(F.softmax(pl,1).cpu().numpy()); _gs.append(torch.sigmoid(gl).cpu().numpy())
            pwm=np.mean(_ps,0); pwm=pwm/pwm.sum(1,keepdims=True); gate=np.mean(_gs,0)
    ...
    out = "figures_v24_ensemble/figure1c_best_logos/figure1c_best_logos"
    ...
    out2 = "figures_v24_ensemble/figure1c_best_logos/figure1c_best_logos_no_deeppbs"
