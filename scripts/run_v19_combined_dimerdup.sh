...
SEED="${SEED:-42}"
EPOCHS="${EPOCHS:-225}"
OUT_ROOT="${OUT_ROOT:-/data1/leihuang/project/TFScope/checkpoints/v19_combined_dimerdup}"
...
command=(
    ...
    scripts/train.py
    --data data/processed/tf_pwm_combined_fm_deeppbs_dimerdup.parquet
    --split data/processed/splits/combined_fm_deeppbs/split.json
    --out "$OUT_DIR"
    --seed "$SEED"
    --epochs "$EPOCHS"
    ...
    --benchmark-eval
    --benchmark-data data/processed/tf_pwm_deeppbs_only_canon_trim_dimerdup.parquet
