...
OUTF="figures_v24_ensemble/figure1e_heldout_perfamily"; RES=OUTF; os.makedirs(RES,exist_ok=True)
...
models=load_ensemble(); print(f"ensemble members: {len(models)}")
rows=[]
for _,r in te.iterrows():
    ...
    g,pp,_=infer_ens(models,tok,mask,int(r.family_id),ret=None)
...
ax.set_title(f"v24-ensemble: benchmark recovery by family (deeppbs_cluster40 test, {len(df)} samples / {te.tf_name.nunique()} TFs)",
