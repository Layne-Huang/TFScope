...
SEED="${SEED:-42}"
EPOCHS="${EPOCHS:-225}"
OUT_ROOT="${OUT_ROOT:-/data1/leihuang/project/TFScope/checkpoints/v19_combined_perprotein_text}"
...
command=(
    ...
    scripts/train.py
    --data data/processed/tf_pwm_combined_perprot.parquet
    --split data/processed/splits/combined_fm_deeppbs/split.json
    --out "$OUT_DIR"
    --seed "$SEED"
    --epochs "$EPOCHS"
    ...
    --num-families 4384
    --family-embedding-path /afs/csail.mit.edu/u/l/leihuang/project/TFScope/data/processed/perprotein_text_embeddings.pt
    --precision bf16
    --tf32
    --no-wandb
)
