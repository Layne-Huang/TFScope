...
# ── (3) PyMOL scripts: interactive (all top-N labelled) + render (clean ray-trace) ──
sel = "+".join(str(r) for r in sorted(top_resnums))
# contact-making top hits get sticks+labels in the render (keeps the figure clean)
contact_top = sorted(r["resnum"] for r in rows_out[:TOPN] if r["crystal_contact"])
csel = "+".join(str(r) for r in contact_top) if contact_top else sel
...
with open(f"{OUT}/{GENE}_render.pml", "w") as o:
    o.write(f"""# TFScope Fig 2b render — {GENE} ({PDBID}:{CHAIN})  (headless: pymol -cq this.pml)
load {os.path.abspath(outpdb)}, {GENE}
bg_color white
hide everything
set ray_shadows, 0
...
show cartoon, polymer.protein and chain {CHAIN}
spectrum b, blue_white_red, polymer.protein and chain {CHAIN}
...
select hits, chain {CHAIN} and resi {csel}
...
turn y, 6
ray 2200, 1700
png {os.path.abspath(OUT)}/{GENE}_render.png, dpi=300
""")
