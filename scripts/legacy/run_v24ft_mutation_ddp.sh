...
BATCH_SIZE="${BATCH_SIZE:-6}"; ACCUM="${ACCUM:-1}"; SEED="${SEED:-42}"; EPOCHS="${EPOCHS:-99}"
DATA="${DATA:-data/processed/tf_pwm_training_v24ft.parquet}"
SPLIT="${SPLIT:-data/processed/splits/train_v24ft/split.json}"
OUT_ROOT="${OUT_ROOT:-/data1/leihuang/project/TFScope/checkpoints/v24ft_mutation}"
PYTHON_BIN="${PYTHON_BIN:-/data1/leihuang/miniconda3/envs/tfscope/bin/python}"
TORCH_HOME="${TORCH_HOME:-/data1/leihuang/.cache/torch}"
RUN_NAME="${RUN_NAME:-v24ft_seed${SEED}}"
...
args=(
  ...
  --oracle-aggregation gene --early-stop-patience 40 --resume /data1/leihuang/project/TFScope/checkpoints/v24_contact/contact_v24_seed42/ckpt_best.pt
