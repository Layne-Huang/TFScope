#!/bin/bash
# Submit the next pending DeepPBS-5fold job on gpu_test.
# gpu_test allows only 2 jobs total; run this after each fold finishes.
# Usage: bash scripts/submit_next_fold.sh
...
for FOLD in 0 1 2 3 4; do
    ...
    RUNNING=$(squeue -u $USER -h 2>/dev/null | grep "5fold_${FOLD}" | wc -l)
    if [ "$RUNNING" -gt 0 ]; then
        echo "fold $FOLD: already running/queued"
        continue
    fi

    echo "Submitting fold $FOLD on gpu_test ..."
    mkdir -p $LOG_OUT
    sbatch \
        --job-name="tfscope_5fold_${FOLD}" \
        --partition=gpu_test --gres=gpu:1 -c 8 --mem=48G --time=8:00:00 \
...
set -eo pipefail
echo '=== DeepPBS fold ${FOLD} | Job \$SLURM_JOB_ID on \$SLURM_NODENAME ==='
nvidia-smi || true
cd $PROJECT
...
echo "All DeepPBS-5fold folds done or queued."
