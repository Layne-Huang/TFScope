#!/usr/bin/env python
"""Re-run DBD domain extraction for aug-set rows with dbd_count=0, using an
expanded Pfam/InterPro whitelist covering families the original 7-family list
(C2H2/bHLH/Homeodomain/bZIP/Nuclear_Receptor/Forkhead/ETS) missed entirely --
Rel-homology/NF-kB, STAT, MADS-box, HMG-box/SOX, GATA, T-box, IRF, p53,
Runt/RUNX, SMAD, Paired-domain/PAX, TEA -- all IDs verified live against the
InterPro API, not guessed from memory.
"""
import sys, time
import requests
import pandas as pd
...
from map_tf_annotations import fetch_interpro_domains, extract_dbd_regions
...
EXPANDED_PFAM = {
    ...
    # newly added, verified live against InterPro API
...
unique_genes = no_domain[["gene_symbol", "uniprot_id", "seq_length"]].drop_duplicates(subset=["gene_symbol"])
...
for i, (_, row) in enumerate(unique_genes.iterrows()):
    gene, uid, seq_len = row["gene_symbol"], row["uniprot_id"], int(row["seq_length"])
    try:
        domains = fetch_interpro_domains(session, uid)
        merged, hit_families = extract_dbd_regions_expanded(domains, seq_len)
...
import json
with open("/tmp/dbd_reextraction_results.json", "w") as f:
    json.dump(results, f)
print("saved to /tmp/dbd_reextraction_results.json", flush=True)
