...
ANCHOR_PATH="${ANCHOR_PATH:-results/v19_e5_registration/structural_anchors_train.tsv}"
REGISTER_LOSS_WEIGHT="${REGISTER_LOSS_WEIGHT:-0.5}"
RUN_NAME="${RUN_NAME:-e5_structural_rag_seed${SEED}}"
...
command=(
    ...
    "$ANCHOR_PATH"
    --register-head
    --register-loss-weight "$REGISTER_LOSS_WEIGHT"
)
...
echo "Starting E5 structural-register RAG seed $SEED on GPUs $GPU_IDS"
echo "Log: $LOG"
