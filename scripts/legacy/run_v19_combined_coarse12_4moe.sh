...
SEED="${SEED:-42}"
EPOCHS="${EPOCHS:-225}"
OUT_ROOT="${OUT_ROOT:-/data1/leihuang/project/TFScope/checkpoints/v19_combined_coarse12_4moe}"
...
command=(
    ...
    scripts/train.py
    --data data/processed/tf_pwm_combined_coarse.parquet
    --split data/processed/splits/combined_fm_deeppbs/split.json
    --out "$OUT_DIR"
    --seed "$SEED"
    --epochs "$EPOCHS"
    ...
    --num-families 12
    --num-experts 4
    --diversity-loss-weight 0
    --family-embedding-path /afs/csail.mit.edu/u/l/leihuang/project/TFScope/data/processed/family_embeddings_coarse.pt
