...
SEED="${SEED:-42}"
EPOCHS="${EPOCHS:-100}"                     # step-matched to E2's 1400 steps (1345 genes / 99 batch = 14 steps/epoch)
OUT_ROOT="${OUT_ROOT:-/data1/leihuang/project/TFScope/checkpoints/v19_deploy_e2_full}"
...
command=(
    ...
    --split  data/processed/splits/cluster40_clean/split_deploy.json
    --out    "$OUT_DIR"
    --seed   "$SEED"
    --epochs "$EPOCHS"
    ...
    --retrieval-index-path data/processed/tf_nn_index_cluster40_full_deploy.json
)
