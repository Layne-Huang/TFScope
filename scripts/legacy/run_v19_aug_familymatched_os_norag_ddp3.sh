...
#
# Split: data/processed/splits/aug_familymatched_os/split.json
...
SEED="${SEED:-42}"
EPOCHS="${EPOCHS:-60}"
OUT_ROOT="${OUT_ROOT:-/data1/leihuang/project/TFScope/checkpoints/v19_aug_familymatched_os_norag}"
...
command=(
    ...
    scripts/train.py
    --data data/processed/tf_pwm_aug_familymatched_os.parquet
    --split data/processed/splits/aug_familymatched_os/split.json
    --out "$OUT_DIR"
    --seed "$SEED"
    --epochs "$EPOCHS"
