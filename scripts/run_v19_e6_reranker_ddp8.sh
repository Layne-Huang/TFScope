...
SEED="${SEED:-42}"
EPOCHS="${EPOCHS:-50}"
OUT_ROOT="${OUT_ROOT:-/data1/leihuang/project/TFScope/checkpoints/v19_e6_reranker_lora_fixed_ddp8}"
INIT_MODEL="${INIT_MODEL:-/data1/leihuang/project/TFScope/checkpoints/v19_e2_gene_balanced_lora_fixed_ddp8/rag_lora_fixed_seed42/ckpt_best.pt}"
...
RUN_NAME="${RUN_NAME:-e6_reranker_seed${SEED}}"
...
if [[ ! -f "$INIT_MODEL" ]]; then
    echo "Missing valid E2 initialization checkpoint: $INIT_MODEL" >&2
    exit 2
fi
...
echo "Starting E6 reranker seed $SEED on GPUs $GPU_IDS"
echo "Log: $LOG"
