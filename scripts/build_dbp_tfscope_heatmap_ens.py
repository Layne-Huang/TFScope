...
os.environ["TORCH_HOME"]="/data1/leihuang/.cache/torch"; os.environ["TRANSFORMERS_OFFLINE"]="1"; os.environ.setdefault("CUDA_VISIBLE_DEVICES","0")
...
TAU=a.tau; VMAX=a.vmax; dev="cuda"
CKDIR="/data1/leihuang/project/TFScope/checkpoints/v24_contact/contact_v24_seed42"
OUT="figures_v24_ensemble/figure_dbp_heatmap"; os.makedirs(OUT,exist_ok=True)
...
_ENSC=[CKDIR+"/ckpt_best.pt"]+[f"checkpoints/iclr_phase1/v24_ens/seed{s}/ckpt_best.pt" for s in (1,7,13,23)]
_MODELS=[]
for _ck in _ENSC:
    _mm=TFScopeModel(cfg).to(dev).eval(); _mm.load_state_dict(torch.load(_ck,map_location=dev,weights_only=False)["model"],strict=False); _MODELS.append(_mm)
print(f"ensemble members: {len(_MODELS)}")
@torch.no_grad()
def predict_full(seq,fid):
    ...
    Ps=[]; Gs=[]
    for _m in _MODELS:
        gl,pl,_=_m(t,dm,fi,retrieved_pwms=None,retrieved_masks=None,retrieved_sims=None,recog_prior=None)
        z=pl[0].cpu().numpy()/TAU; z=z-z.max(0,keepdims=True); P=np.exp(z); P/=P.sum(0,keepdims=True)
        Ps.append(P); Gs.append(gl.sigmoid()[0].cpu().numpy())
    P=np.mean(Ps,0); P/=P.sum(0,keepdims=True)
    return P, np.mean(Gs,0)
